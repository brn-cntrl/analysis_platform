...
function ExternalConfigStep({
  externalFilesBySubject,
  selectedSubjects,
  externalConfigs,
  setExternalConfigs
}) {
  ...
  const [externalMode, setExternalMode] = useState('union');
  ...
  const experimentTypeMap = useMemo(() => {
    ...
    selectedSubjectsList.forEach(subject => {
      const files = externalFilesBySubject[subject] || [];
    ...
    return map;
  }, [externalFilesBySubject, selectedSubjects]);
  ...
  }, [experimentTypeMap, externalMode, selectedSubjects]);
  ...
  useEffect(() => {
    ...
    Object.keys(experimentTypeMap).forEach(expType => {
      ...
      if (firstFile && externalConfigs?.[firstSubject]?.[firstFile.filename]?.selected) {
  ...
  }, [experimentTypeMap, externalConfigs]);
  ...
  const getExperimentConfig = (expType) => {
    ...
    return externalConfigs?.[firstSubject]?.[firstFile.filename] || getDefaultConfig();
  };
  ...
  const selectedSubjectsWithExternal = Object.keys(selectedSubjects)
    .filter(s => selectedSubjects[s] && externalFilesBySubject[s]);

  if (selectedSubjectsWithExternal.length === 0) {
    return (
      <div className="wizard-section external-config-section">
        <h3 className="wizard-section-title">External Data Configuration</h3>
        <p className="wizard-section-description">
          No External data found for the selected subjects.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="wizard-section external-config-section">
      <h3 className="wizard-section-title">External Data Configuration</h3>
      <p className="wizard-section-description">
        Configure how to interpret your External CSV data files for analysis. Select experiment types and configure them once to apply to all subjects.
      </p>

      {/* Include external files toggle */}
      <div className="external-include-toggle">
      ...
      {includeExternal && (
        <>
          ...
          {selectedSubjectsWithExternal.length > 1 && (
            <div className="external-mode-toggle">
              <label className="mode-option">
                <input
                  type="radio"
                  name="externalMode"
                  value="union"
                  checked={externalMode === 'union'}
              ...
              <label className="mode-option">
                <input
                  ...
                  name="externalMode"
                  value="intersection"
                  checked={externalMode === 'intersection'}
                  onChange={(e) => setExternalMode(e.target.value)}
                />
          ...
          <div className="external-selection-panel">
            ...
            <div className="experiment-type-checkboxes">
              {availableExperimentTypes.length === 0 ? (
                ...
                availableExperimentTypes.map(expType => {
                  const data = experimentTypeMap[expType];
                  const totalSubjects = selectedSubjectsWithExternal.length;
                  ...
                  return (
                    <label key={expType} className="experiment-checkbox-item">
                      ...
                      {externalMode === 'union' && totalSubjects > 1 && (
          ...
          {Object.keys(selectedExperimentTypes).filter(exp => selectedExperimentTypes[exp]).map(expType => {
            ...
            return (
              <div key={expType} className="external-experiment-card">
                <div 
                  className="external-experiment-header"
                  onClick={() => toggleExperimentExpansion(expType)}
                >
                  <span className="external-experiment-expand-icon">
                    {isExpanded ? '▼' : '▶'}
                  </span>
                  <div className="external-experiment-info">
                    <span className="external-experiment-name">{expType}</span>
                    <span className="external-experiment-subjects">
                      Applies to: {data.subjects.join(', ')}
                    </span>
                    <span className="external-experiment-stats">
          ...
          <div className="external-overall-summary">
            <h4 className="summary-title">Overall External Data File Configuration</h4>
